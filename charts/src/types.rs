...
/// Persistent candlestick entities with dynamic pooling
/// Grows as needed when zooming out, reuses entities when zooming in
#[derive(Resource)]
pub struct PersistentCandlestickEntities {
    pub wicks: Vec<Entity>,   // Pool of wick entities
    pub bodies: Vec<Entity>,  // Pool of body entities
}

impl PersistentCandlestickEntities {
    pub fn new() -> Self {
        Self {
            wicks: Vec::new(),
            bodies: Vec::new(),
    ...
    /// Ensure we have enough entities for the visible candle count
    /// Spawns new entities if needed (dynamic growth)
    pub fn ensure_capacity(&mut self, commands: &mut Commands, needed: usize) {
        while self.wicks.len() < needed {
            let wick = commands.spawn((
                SpriteBundle {
                    sprite: Sprite {
                        color: Color::srgb(0.5, 0.5, 0.5),
                        custom_size: Some(Vec2::new(1.0, 10.0)),
                        ..default()
                    },
                ...
                CandlestickWick { candle_index: 0 },
                PriceElement,
                PaneId::Price,
            )).id();
            ...
            let body = commands.spawn((
                ...
                CandlestickBody { candle_index: 0 },
                PriceElement,
                PaneId::Price,
            )).id();

            self.wicks.push(wick);
            self.bodies.push(body);
        }
    }
}

/// Persistent volume bar entities with dynamic pooling
#[derive(Resource)]
pub struct PersistentVolumeEntities {
    pub bars: Vec<Entity>,  // Pool of volume bar entities
}

impl PersistentVolumeEntities {
    pub fn new() -> Self {
        Self {
            bars: Vec::new(),
        }
